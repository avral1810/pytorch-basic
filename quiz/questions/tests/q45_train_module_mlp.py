...
def get_visible_tests():
    return [
        ("returns a float validation accuracy", test_returns_float),
        ("learns the task well", test_good_accuracy),
    ]
...
def test_returns_float(solution_module):
    actual = solution_module.train_module_mlp(42)
    assert isinstance(actual, float), f"Expected a float, got {type(actual)}"


def test_good_accuracy(solution_module):
    actual = solution_module.train_module_mlp(42)
    assert actual >= 0.9, f"Expected validation accuracy >= 0.9, got {actual:.3f}"


def test_deterministic(solution_module):
    first = solution_module.train_module_mlp(42)
    second = solution_module.train_module_mlp(42)
...
def test_accuracy_range(solution_module):
    actual = solution_module.train_module_mlp(7)
